<script>
	...
	import { getData } from '$lib/eventsQuery';
	...
	let filteredArticles = $derived(articles.filter(article => 
		article.event_title.toLowerCase().includes(searchTerm.toLowerCase()) ||
...
<svelte:head>
	<title>Events - Spring Side Academy</title>
	<meta name="description" content="Join us at Spring Side Academy for our upcoming events." />
</svelte:head>
					...
					>
						<div class="relative h-64 w-full overflow-hidden bg-slate-200">
							...
							{#if article.event_date}
								...
								>
									<span class="text-lg leading-none text-slate-900">{article.event_date.split(' ')[0]}</span>
									<span class="text-[10px] tracking-tighter text-[#0f2a92] uppercase"
										>{article.event_date.split(' ')[1] || ''}</span
									>
								</div>
							{/if}
							<!-- <div
						...
						<div class="flex flex-1 flex-col p-8">
							...
							<div class="mt-auto border-t border-slate-200 pt-6">
								<a href={`/events/${article.slug || '#'}`} class="cursor-pointer">
									<button
