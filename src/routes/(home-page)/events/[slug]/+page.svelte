<script>
	...
	import { getEventBySlug } from '$lib/eventsQuery';
	...
	async function loadArticle(currentSlug) {
		...
		error = null;
		try {
			const result = await getEventBySlug(currentSlug);
			if (result.error) {
				error = result.error;
			} else {
				article = result.data;
...
<svelte:head>
	<title>{article?.event_title || 'Event'} - Spring Side Academy</title>
</svelte:head>
    ...
    <div class="mx-auto max-w-4xl px-6 lg:px-8">
        ...
        {:else if error}
            <div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl shadow-sm text-center px-6">
                <div class="text-red-500 mb-4 text-lg font-semibold">Unable to load event</div>
                <p class="text-slate-500 mb-6">We encountered an error while fetching the event details.</p>
            ...
            <article class="bg-white rounded-3xl shadow-sm overflow-hidden">
                ...
                <div class="p-8 md:p-12">
                     <div class="flex items-center gap-4 text-sm text-slate-500 mb-8 border-b border-gray-100 pb-8">
                        <span class="font-medium text-slate-900">Event Date: {article.event_date}</span>
                        <span>•</span>
        ...
        {:else}
             <div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl shadow-sm text-center px-6">
                <h2 class="text-2xl font-bold text-slate-900 mb-2">Event Not Found</h2>
                <p class="text-slate-500 mb-6">The event you are looking for does not exist or has been removed.</p>
                <a href="/events" class="px-6 py-2 bg-[#0f2a92] text-white rounded-lg hover:bg-[#0f2a92]/90 transition-colors">
                    Browse All Events
                </a>
            </div>
        {/if}
